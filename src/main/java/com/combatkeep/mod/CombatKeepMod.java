	...
	public void onInitialize() {
		...
		LOGGER.info("  - Solidus economy integration: {}",
			SolidusIntegration.isEnabled()
				? "ENABLED (" + (long) penaltyPercent + "% balance penalty)"
					...
					if (server != null) {
						...
						if (killer != null) {
							...
							if (SolidusIntegration.isEnabled()) {
								penaltyAmount = SolidusIntegration.applyDeathPenalty(player, killer);
							}
			...
			if (isCombatTagged(player)) {
				...
				if (opponentUuid != null) {
					...
					if (opponent != null) {
						...
						// ── Apply Solidus economy penalty on combat log ──
						double penaltyAmount = 0.0;
						if (SolidusIntegration.isEnabled() && CombatKeepConfig.getInstance().isSolidusCombatLogPenalty()) {
							penaltyAmount = SolidusIntegration.applyCombatLogPenalty(player, opponent);
						}
